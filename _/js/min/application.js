...
function createYoutube(){var ytWrap=document.createElement('div');playerElement.appendChild(ytWrap);player=new YT.Player(ytWrap,{videoId:videoId,origin:window.location.origin,rel:0,playerVars:{"showinfo":0,"color":"white","rel":0,"autoplay":1},events:{"onReady":youtubeReady,"onStateChange":youtubeStateChange}});}
...
function vimeoMessageReceived(e){var data=JSON.parse(e.data);switch(data.event){case'ready':post('addEventListener','finish');post('setVolume',0);break;case'finish':videoEnded();break;}}
return Module;}());var App=(function(){"use strict";var currentVideoIndex;var currentVideo;var videos;var Module={init:function(options){videos=options.videos;var randomVideoIndex=0;createVideo(randomVideoIndex);$(window).on('videoEnded',function(){videoEnded();});}}
function createVideo(i){console.log('createVideo('+i+')');currentVideo=Object.create(Video);currentVideo.init({video:videos[i]});currentVideoIndex=i;}
function nextVideo(){console.log('this.nextVideo');createVideo(currentVideoIndex+1);}
function videoEnded(){if(i<videos.length){nextVideo();}else{createVideo(0);}}
